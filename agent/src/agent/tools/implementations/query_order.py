...
class QueryOrderArgs(BaseModel):
    ...
    status: int | None = Field(None, description="Filter by status: 0=pending, 1=paid, 2=shipped, 3=received")
    page: int = Field(1, description="Page number")


class QueryOrderTool(SmartDayBaseTool):
    name: str = "query_order"
    description: str = "Query order status by order ID or list user's orders filtered by status"
