...
class TestCouponSchema:
    ...
    def test_coupon_create_date_validation(self):
        ...
        with pytest.raises(ValidationError):
            CouponCreate(name="Test", amount=Decimal("10"), count=10,
    ...
    def test_coupon_history_response(self):
        ...
        h = CouponHistoryResponse(
            id=uuid.uuid4(), coupon_id=uuid.uuid4(), user_id=uuid.uuid4(),
            coupon_name="Test", coupon_amount=Decimal("10"), coupon_min_amount=Decimal("50"),
            use_status=0, receive_time=datetime.now(), expire_time=datetime.now(),
        )
...
class TestFlashSchema:
    ...
    def test_flash_session_create(self):
        ...
        now = datetime.now()
        s = FlashSessionCreate(promotion_id=uuid.uuid4(), name="10点场",
    ...
    def test_flash_product_create(self):
        ...
        p = FlashProductCreate(
            session_id=uuid.uuid4(), product_id=uuid.uuid4(), sku_id=uuid.uuid4(),
            flash_price=Decimal("99.00"), flash_stock=500,
        )
...
class TestMigration:
    def test_phase4_migration_exists(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "phase4_migration", "alembic/versions/d4e5f6a7b8c9_create_commerce_promotion_tables.py",
        )
