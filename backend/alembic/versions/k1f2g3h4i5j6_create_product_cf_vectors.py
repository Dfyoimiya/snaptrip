...
def upgrade() -> None:
    ...
    # 唯一索引: 一个商品只有一个 CF 向量
    op.create_index("ix_pms_product_cf_vectors_product_id", "pms_product_cf_vectors",
                    ["product_id"], unique=True)

    # 外键: 级联删除
    op.create_foreign_key(
        "fk_pms_product_cf_vectors_product_id",
        "pms_product_cf_vectors", "pms_products",
        ["product_id"], ["id"],
        ondelete="CASCADE",
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint("fk_pms_product_cf_vectors_product_id", "pms_product_cf_vectors",
                       type_="foreignkey")
