...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "priority", sa.String(16), nullable=False, server_default="normal", comment="优先级: normal/urgent/critical"
        ),
        ...
        sa.Column("assigned_agent_id", postgresql.UUID(as_uuid=True), nullable=True, comment="指派的坐席 admin ID"),
        sa.Column("sla_deadline", sa.DateTime(timezone=True), nullable=True, comment="SLA 响应截止时间"),
        sa.Column("first_response_at", sa.DateTime(timezone=True), nullable=True, comment="首次人工响应时间"),
    ...
    op.create_index("ix_oms_support_tickets_assigned_agent_id", "oms_support_tickets", ["assigned_agent_id"])
    ...
    op.create_table(
        ...
        sa.Column("intent", sa.String(32), nullable=True, comment="客服意图: cs_after_sales/cs_complaint/cs_inquiry"),
        ...
        sa.Column("conversation_turns", sa.Integer, nullable=False, server_default="0", comment="对话轮数"),
        sa.Column("tools_called", postgresql.JSON, nullable=True, comment="调用的工具列表"),
        sa.Column(
            "key_entities", postgresql.JSON, nullable=True, comment="关键实体: {order_ids, product_ids, amounts, ...}"
        ),
        sa.Column("emotion_trajectory", sa.String(32), nullable=True, comment="情绪轨迹: angry→calm"),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("created_by", postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column("updated_by", postgresql.UUID(as_uuid=True), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("ticket_id", postgresql.UUID(as_uuid=True), nullable=False, comment="关联工单ID"),
        sa.Column("sender_type", sa.String(16), nullable=False, comment="发送者类型: user/agent/system"),
        sa.Column("sender_id", postgresql.UUID(as_uuid=True), nullable=True, comment="发送者 ID"),
    ...
    op.create_index("ix_cs_conversation_messages_ticket_id", "cs_conversation_messages", ["ticket_id"])
    ...
    op.create_table(
        "cs_agent_status",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("admin_id", postgresql.UUID(as_uuid=True), nullable=False, comment="关联 users 表的 admin ID"),
        sa.Column(
            "status", sa.String(16), nullable=False, server_default="offline", comment="坐席状态: online/offline/busy"
        ),
        sa.Column("current_ticket_id", postgresql.UUID(as_uuid=True), nullable=True, comment="当前正在处理的工单ID"),
        sa.Column("max_concurrent", sa.Integer, nullable=False, server_default="3", comment="最大同时处理工单数"),
    ...
    op.create_table(
        "cs_notifications",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("recipient_id", postgresql.UUID(as_uuid=True), nullable=False, comment="接收通知的管理员 ID"),
        sa.Column(
            "type", sa.String(32), nullable=False, comment="通知类型: new_ticket/new_message/sla_warning/sla_breach"
        ),
        sa.Column("ticket_id", postgresql.UUID(as_uuid=True), nullable=True, comment="关联工单ID"),
