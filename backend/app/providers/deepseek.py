...
from app.schemas.llm import ChatResult, ModelPricing, StreamChunk, TokenUsage
...
class DeepSeekProvider(BaseLLMProvider):
    ...
    async def chat(
        ...
        try:
            ...
            raise LLMError(
                f"DeepSeek API 返回非 JSON 响应: {resp.text[:500]}",
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout) as client, client.stream(
                ...
                if resp.status_code != 200:
                    ...
                    raise LLMError(
                        f"DeepSeek API 返回 {resp.status_code}: {body[:500]}",
                        details={"provider": "deepseek", "http_status": resp.status_code},
                    )
