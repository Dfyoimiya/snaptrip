...
class CmsService:
    ...
    async def list_banners(self, status: int | None = None) -> list[BannerResponse]:
        from app.models.cms.content import CmsBanner
    ...
    async def list_subjects(self, status: int | None = None, page: int = 1, page_size: int = 20
    ...
    async def list_helps(self, category_name: str | None = None, status: int | None = None,
                         page: int = 1, page_size: int = 20) -> tuple[list[HelpResponse], int]:
        from app.models.cms.content import CmsHelp
        base = select(CmsHelp)
...
class StatsService:
    ...
    async def get_dashboard_overview(self) -> DashboardOverview:
        ...
        on_shelf_result = await self.db.execute(
            select(func.count(PmsProduct.id)).where(
                PmsProduct.publish_status == 1, PmsProduct.is_deleted.is_(False)
            )
        )
    ...
    async def get_sales_stats(self, days: int = 7) -> list[SalesStatItem]:
        ...
        result = await self.db.execute(
            select(
                ...
                OmsOrder.delete_status == 0,
            ).group_by(func.date(OmsOrder.created_at)).order_by(func.date(OmsOrder.created_at).asc())
        )
        rows = result.all()
        return [
            SalesStatItem(date=str(r.date), order_count=int(r.cnt), amount=float(r.amt))
            for r in rows
        ]
    ...
    async def get_homepage(self) -> HomePageAggregation:
        ...
        banner_result = await self.db.execute(
            select(CmsBanner).where(CmsBanner.status == 1).order_by(CmsBanner.sort.asc()).limit(5)
        )
        ...
        rec_result = await self.db.execute(
            select(PmsProduct)
            .where(
                PmsProduct.publish_status == 1, PmsProduct.verify_status == 1,
                PmsProduct.recommend_status == 1, PmsProduct.is_deleted.is_(False),
            )
            .order_by(PmsProduct.sale_count.desc())
            .limit(8)
        )
        ...
        return HomePageAggregation(
            banners=banners, new_products=new_products,
            recommend_products=recommend_products, subjects=subjects,
        )
