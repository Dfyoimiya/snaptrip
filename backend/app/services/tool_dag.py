...
class ToolDAGExecutor:
    ...
    async def _execute_node(self, inv: ToolInvocation) -> ToolResult:
        ...
        return await breaker.call(_call, invocation_id=inv.invocation_id)
    ...
    async def _http_call(self, inv: ToolInvocation) -> ToolResult:
        ...
        try:
            client = self.http
            if method == "GET":
                resp = await client.get(path, params=inv.params)
            else:
                resp = await client.post(path, json=inv.params)
            ...
            return ToolResult(
                ...
                status="timeout",
                error_code="TIMEOUT",
                error_message=f"调用 {inv.tool_name} 超时",
                latency_ms=elapsed,
            )
