...
async def create(data: CouponCreate, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)):
...
async def delete(coupon_id: UUID, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)):
    svc = CouponService(db)
...
async def get_detail(coupon_id: UUID, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)):
    svc = CouponService(db)
