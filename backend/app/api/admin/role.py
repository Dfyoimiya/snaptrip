...
async def list_paginated(
    ...
    result = await db.execute(
        select(Role).order_by(Role.sort.asc()).offset((page - 1) * page_size).limit(page_size)
    )
    roles = result.scalars().all()
...
async def list_menu(
    ...
    return success([
        {"id": m.id, "parent_id": m.parent_id, "title": m.title, "name": m.name, "sort": m.sort}
        for m in menus
    ])
...
async def list_resource(
    ...
    return success([
        {"id": r.id, "category_id": r.category_id, "name": r.name, "url": r.url}
        for r in resources
    ])
