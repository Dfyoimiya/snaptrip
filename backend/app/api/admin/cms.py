...
async def list_banners(
    ...
    position: str | None = Query(None, description="筛选位置: HOME_TOP/HOME_MIDDLE"),
    db: AsyncSession = Depends(get_db),
    _u=Depends(require_admin_user),
):
    svc = CmsService(db)
    items = await svc.list_banners(status=status, position=position)
    return success([i.model_dump() for i in items])


@cms_router.post("/banners", summary="新增轮播图", status_code=201)
async def create_banner(data: BannerCreate, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)):
    svc = CmsService(db)
...
async def update_banner(
    banner_id: UUID, data: BannerUpdate, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)
):
    svc = CmsService(db)
...
async def delete_banner(banner_id: UUID, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)):
    svc = CmsService(db)
...
async def sort_banner(
    banner_id: UUID, sort: int = Query(..., ge=0), db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)
):
    svc = CmsService(db)
...
async def create_subject(data: SubjectCreate, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)):
    svc = CmsService(db)
...
async def update_subject(
    subject_id: UUID, data: SubjectUpdate, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)
):
    svc = CmsService(db)
...
async def delete_subject(subject_id: UUID, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)):
    svc = CmsService(db)
...
async def create_help(data: HelpCreate, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)):
    svc = CmsService(db)
...
async def update_help(
    help_id: UUID, data: HelpUpdate, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)
):
    svc = CmsService(db)
...
async def delete_help(help_id: UUID, db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)):
    svc = CmsService(db)
...
async def dashboard_overview(db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)):
    svc = StatsService(db)
...
async def sales_stats(
    days: int = Query(7, ge=1, le=365), db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)
):
    svc = StatsService(db)
...
async def product_rank(
    limit: int = Query(10, ge=1, le=100), db: AsyncSession = Depends(get_db), _u=Depends(require_admin_user)
):
    svc = StatsService(db)
