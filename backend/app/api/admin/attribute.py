...
async def list_categories_with_attrs(
    ...
    result = await db.execute(select(PmsProductAttribute).order_by(PmsProductAttribute.sort.asc()))
    ...
    cat_names: dict = {}
    if cat_ids_set:
        result = await db.execute(select(PmsCategory).where(PmsCategory.id.in_(cat_ids_set)))
...
async def list_paginated(
    ...
    result = await db.execute(
        base.order_by(PmsProductAttribute.sort.asc()).offset((page - 1) * page_size).limit(page_size)
    )
