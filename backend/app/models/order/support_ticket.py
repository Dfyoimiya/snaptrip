...
class OmsSupportTicket(CommerceBase, AuditMixin):
    ...
    member_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), nullable=False, index=True, comment="会员ID"
    )
    type: Mapped[str] = mapped_column(
        String(32), default="inquiry", nullable=False, index=True,
    ...
    title: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="工单标题"
    )
    ...
    tags: Mapped[list | None] = mapped_column(
        JSON, nullable=True, comment="工单标签: ['urgent', 'vip']"
    )
