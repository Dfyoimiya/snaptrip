...
marketplace 本地生活服务模型。agent_worker 跨包 relationship（如 Plan ↔ Checkpoint）
在 marketplace 模型全部加载后再导入 agent_worker 模型，以避免循环导入。
...
# 跨包 relationship 解析 —— 必须在 marketplace 模型全部导入之后再 import agent_worker 模型，
# 否则 SQLAlchemy mapper 配置阶段会因找不到 "Checkpoint" 而失败。
# ruff: noqa: I001  — import 顺序有意为之，不能打乱。
from agent_worker.app.agent.models.checkpoint import Checkpoint  # noqa: F401, E402
