...
async def seed(session=None):
    ...
    products = [
        p("iPhone 15 Pro Max", Decimal("9999.00"), "Apple", cat_ids["智能手机"],
    ...
    for name, price, brand, cat, skus, sub, original, is_new, is_rec, desc in products:
        ...
        await session.execute(text(
            ...
            "id": pid, "name": name, "price": str(price), "orig": str(original) if original else None,
            "brand": brand_ids[brand], "cat": cat, "stock": total_stock, "new": is_new, "rec": is_rec,
            "sub": sub, "desc": desc,
