...
/**
 * ============================================
 * 我的收藏 (MemberFavoritesView)
 * 浏览足迹式布局：日期分组 + 方形图片卡片 + 搜索 + 批量管理
 * 支持时间排序（最近/最早）+ 商品状态筛选（上架/下架）
 * ============================================
 */
...
const allItems = ref<MemberProductCollection[]>([])
...
const filteredItems = computed(() => {
  ...
  if (statusFilter.value !== undefined) {
    items = items.filter(item => item.productStatus === statusFilter.value)
  }
  ...
  items.sort((a, b) => {
    ...
    return sortOrder.value === 'desc' ? db.localeCompare(da) : da.localeCompare(db)
  })
...
const groupedByDate = computed(() => {
  ...
  return Array.from(dateMap.entries())
    .sort((a, b) => sortOrder.value === 'desc' ? b[0].localeCompare(a[0]) : a[0].localeCompare(b[0]))
    .map(([dateKey, items]) => ({
...
const isAllSelected = computed(() => {
  if (visibleIds.value.size === 0) return false
...
async function loadFavorites() {
  ...
  try {
    ...
    allItems.value = ((res as unknown as { items: MemberProductCollection[] }).items || [])
      .map((item: any) => ({
        ...item,
        productStatus: item.productStatus ?? item.product_status,
      }))
...
async function batchDelete() {
  ...
  if (!confirm(`确定删除选中的 ${selectedIds.value.size} 件收藏商品？`)) return
  deleting.value = true
  try {
    for (const id of selectedIds.value) {
      await deleteProductCollectionAPI({ productId: String(id) }).catch(() => {})
    }
    // 从本地移除已删除的
...
async function removeSingle(id: string) {
  ...
  try {
    ...
    console.error('取消收藏失败:', err?.message || err)
  } finally {
    deleting.value = false
  }
}
...
watch([searchKeyword, sortOrder, statusFilter], () => {
  selectedIds.value = new Set()
})
...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden min-h-[500px]">
      ...
      <div v-if="allItems.length > 0" class="flex items-center gap-3 flex-1 justify-end">
        ...
        <div class="flex items-center gap-1 bg-gray-50 rounded-lg p-0.5">
          <button
            ...
            :class="[
              ...
              statusFilter === opt.value ? 'bg-white text-brand-600 shadow-sm font-medium' : 'text-gray-500 hover:text-gray-700',
            ]"
            @click="statusFilter = opt.value as number | undefined"
          >{{ opt.label }}</button>
        </div>
        ...
        <div class="flex items-center gap-1 bg-gray-50 rounded-lg p-0.5">
          <button
            :class="[
              'px-2.5 py-1 text-xs rounded-md transition-colors',
              sortOrder === 'desc' ? 'bg-white text-brand-600 shadow-sm font-medium' : 'text-gray-500 hover:text-gray-700',
            ]"
          ...
          <button
            :class="[
              'px-2.5 py-1 text-xs rounded-md transition-colors',
              sortOrder === 'asc' ? 'bg-white text-brand-600 shadow-sm font-medium' : 'text-gray-500 hover:text-gray-700',
            ]"
            @click="sortOrder = 'asc'"
          >最早</button>
        </div>
    ...
    <div v-else-if="allItems.length === 0" class="flex flex-col items-center justify-center py-20 text-gray-400">
      ...
      <p class="text-base font-medium">暂无收藏商品</p>
      <p class="text-sm mt-1">浏览商品时点击收藏，这里就会出现哦</p>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          ...
          >
            ...
            <div class="aspect-square bg-gray-50 rounded-xl overflow-hidden relative">
              ...
              <!-- 左上角勾选框（常驻） -->
              <label class="absolute top-2 left-2 z-10" @click.stop>
                <input
                  type="checkbox"
              ...
              <!-- 右下角取消收藏按钮（hover 显示） -->
              <button
                class="absolute bottom-2 right-2 z-10 w-7 h-7 bg-white/80 backdrop-blur rounded-full flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-white transition-all opacity-0 group-hover:opacity-100"
                @click.stop="removeSingle(item.id!)"
              >
            ...
            <div class="pt-2">
              ...
              <div class="flex items-baseline gap-2 mt-1">
                ...
                <span
                  ...
                  class="text-xs text-gray-400 border border-gray-200 px-1.5 py-0.5 rounded"
                >已下架</span>
              </div>
            </div>
          </div>
